...
namespace Demo {
	public class SimpleRoof2 : Shape {
...
void CreateFlatRoofPart(GameObject roofPrefab) {
	...
	switch (side) {
		case 0: // depth direction
			for (int i = 0; i < 2; i++) {
				flatRoof = CreateSymbol<SimpleRow2>("roofStrip", new Vector3((Width - 1) * (i - 0.5f), 0, 0));
		...
		case 1: // width direction
			for (int i = 0; i < 2; i++) {
				flatRoof = CreateSymbol<SimpleRow2>("roofStrip", new Vector3(0, 0, (Depth - 1) * (i - 0.5f)));
...
void createFlat(GameObject roofPrefab) {
	...
	if (Width % 2 != 0) {
		for (int i = 0; i < Width; i++) {
			...
			flatRoof.Initialize(Depth, roofPrefab, roofPrefab);
			flatRoof.Generate();
		}
	} else {
		for (int i = 0; i < Width; i++) {
			flatRoof = CreateSymbol<SimpleRow2>("roofStrip", new Vector3((Width / 2 - 0.5f - i), 0, 0));
			flatRoof.Initialize(Depth, roofPrefab, roofPrefab);
			flatRoof.Generate();
		}
	}
